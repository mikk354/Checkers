...
private:
    vector<vector<POS_T>> make_turn(vector<vector<POS_T>> mtx, move_pos turn) const
    {
        ...
        if ((mtx[turn.x][turn.y] == 1 && turn.x2 == 0) || (mtx[turn.x][turn.y] == 2 && turn.x2 == 7)) // проверяет на какие координаты встает шашка, возвращает в виде вектора
            mtx[turn.x][turn.y] += 2;
    ...
    double calc_score(const vector<vector<POS_T>>& mtx, const bool first_bot_color) const
    {
        ...
        double w = 0, wq = 0, b = 0, bq = 0;      // для подсчета шашек\ дамок разных цветов
        for (POS_T i = 0; i < 8; ++i)
        {
            for (POS_T j = 0; j < 8; ++j)
            {
                w += (mtx[i][j] == 1);
                wq += (mtx[i][j] == 3);                        // проход по всем шашкам \ дамкам 
                b += (mtx[i][j] == 2);
                bq += (mtx[i][j] == 4);
                if (scoring_mode == "NumberAndPotential")
                {
                    w += 0.05 * (mtx[i][j] == 1) * (7 - i);          // подсчет 
                    b += 0.05 * (mtx[i][j] == 2) * (i);
                }
            }
        }
        ...
        return (b + bq * q_coef) / (w + wq * q_coef);         // возвращает отношение оставшихся посчитанных шашек и дамок, из этого расчитывает счет
    }
...
private:
  ...
  public:
     ...
     bool have_beats;         // битая ли шашка
     int Max_depth;           // максимальная глубина поиска

  private:
     default_random_engine rand_eng; // случайные числа
     string scoring_mode;            // как будет оцениватся ход
     string optimization;            // как будет выбиратся оптимизация
     vector<move_pos> next_move;     // хранение ходов
     vector<int> next_best_state;    // хранение лучших ходов
